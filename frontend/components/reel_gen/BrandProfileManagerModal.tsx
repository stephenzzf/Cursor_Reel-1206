...
import { extractBrandDNA } from '../../hooks/useReelApi';  // 改为调用后端 API
import { createVisualProfile, updateVisualProfile } from '../../services/brandProfileService';
import { BrandVisualProfile } from '../../types';
import { SparklesIcon, PhotoIcon, VideoCameraIcon } from './ReelGenAssets';  // 从 ReelGenAssets 导入图标
import { PencilIcon } from '../image_gen/ImageGenAssets';  // PencilIcon 从 ImageGenAssets 导入
...
const BrandProfileManagerModal: React.FC<BrandProfileManagerModalProps> = ({ 
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const logoData = logoImage ? await processFile(logoImage) : undefined;
            const refData = referenceImages.length > 0 ? await Promise.all(referenceImages.map(processFile)) : undefined;
            
            // Only include video URLs if not in image mode
            const validVideoUrls = mode !== 'image' ? videoUrls.filter(u => u.trim() !== '') : undefined;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (error?.message?.includes("LIMIT_REACHED") || error?.message?.includes("数量已达上限")) {
    ...
    const getDisplayFields = () => {
        const fields = [
            { label: '视觉风格 (Style)', key: 'visualStyle', color: 'bg-blue-50 text-blue-700 border-blue-100' },
